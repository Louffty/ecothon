...
func (s *eventStorage) GetAllByUserUUID(ctx context.Context, userUUID string, limit, offset int) ([]*entities.Event, error) {
	var events []*entities.Event
	err := s.db.WithContext(ctx).Model(&entities.Event{}).Where("user_uuid = ?", userUUID).Limit(limit).Offset(offset).Find(&events).Error
	return events, err
}
...
func (s *eventStorage) GetUsersEvents(ctx context.Context, uuid string, limit, offset int) ([]*entities.Event, error) {
	var events []*entities.Event
	err := s.db.WithContext(ctx).Model(&entities.Event{}).
