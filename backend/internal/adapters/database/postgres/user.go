...
func (s *usersStorage) GetByName(ctx context.Context, username string) (*entities.User, error) {
	var user *entities.User
	err := s.db.WithContext(ctx).Model(&entities.User{}).Where("username = ?", username).First(&user).Error
	return user, err
}
