...
const MyConference = () => {
  ...
  const handlePatchRequest = async (conference) => {
    try {
      const response = await fetchWithAuth(
        `https://nothypeproduction.space/api/v1/conference/${conference.uuid}/archive`,
        {
          method: "PATCH",
          headers: {
  ...
  const handleLinkSubmit = async () => {
    try {
      await fetchWithAuth(`https://nothypeproduction.space/api/v1/conference/url`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
  ...
  useEffect(() => {
    const fetchConferences = async () => {
      try {
        const response = await fetchWithAuth(
          "https://nothypeproduction.space/api/v1/conference/my",
          {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
